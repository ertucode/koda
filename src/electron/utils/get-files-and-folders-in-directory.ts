...
export async function getFilesAndFoldersInDirectory(
  ...
  try {
    ...
    const items = await Promise.all(
      dirents.map(async (entry): Promise<GetFilesAndFoldersInDirectoryItem> => {
...
export async function getFileInfoByPaths(filePaths: string[]): Promise<GetFilesAndFoldersInDirectoryItem[]> {
  const items = await Promise.all(

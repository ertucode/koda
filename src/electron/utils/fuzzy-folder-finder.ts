...
export async function fuzzyFolderFinder(directory: string, query: string): Promise<GenericResult<string[]>> {
...
function listFoldersRecursively(target: string, signal?: AbortSignal): Promise<GenericResult<string[]>> {
  return new Promise<GenericResult<string[]>>((resolve, reject) => {
    ...
    if (isHomeDir) {
      ...
      // Too expensive in home directory
      args.push('--hidden')
    }

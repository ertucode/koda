...
export const useSyncDirectoryStoreAndLayout = ({ layoutRef }: { layoutRef: React.RefObject<Layout | null> }) => {
  ...
  const handleModelChange = useCallback(() => {
    ...
    if (directoryIds.length !== directoryIdsInStore.length) {
      const direcoryIdToRemove = directoryIdsInStore.find(id => !directoryIds.includes(id))
  ...
  useEffect(() => {
    const unsub = directoryStore.subscribe(s => {
      ...
      layoutModel.visitNodes(node => {
        if (done) return
        if (LayoutHelpers.isDirectory(node) && node.getConfig()?.directoryId === directoryId) {
          done = true
          ...
          if (activeTabset?.getId() === tabset.getId() && activeTab?.getId() === targetTab.getId()) {
            return
          }
          ...
          const idx = tabset.getChildren().findIndex(c => c.getId() === targetTab?.getId())
          if (idx !== -1) {

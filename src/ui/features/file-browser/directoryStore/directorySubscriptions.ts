...
export const directoryDerivedStores = new Map<
  DirectoryId,
  {
    ...
    getFilteredDirectoryData: () => GetFilesAndFoldersInDirectoryItem[] | undefined
    useSort: () => SortState
    ...
    subscribeToStores(
      ...
      ([d]) => {
        ...
        if (s.pendingSelection && filteredDirectoryData.length > 0) {
          const newItemIndex = filteredDirectoryData.findIndex(item => item.name === s.pendingSelection)
          if (newItemIndex !== -1) {

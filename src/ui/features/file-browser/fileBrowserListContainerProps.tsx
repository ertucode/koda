import { DerivedDirectoryItem, DirectoryId, DirectoryInfo } from './directoryStore/DirectoryBase'
import { fileDragDropHandlers } from './fileDragDrop'

// Find the closest row index based on mouse position
function findClosestRowIndex(e: React.MouseEvent, directoryId: DirectoryId): number | null {
...
export function fileBrowserListContainerProps({
  ...
  onContextMenu: (e: React.MouseEvent, item: undefined) => void
  ...
  return {
    onMouseDown: e => {
      ...
      fileDragDropHandlers.startDragToSelect(directoryId, e.metaKey, { x: e.clientX, y: e.clientY }, scrollContainer)
    },

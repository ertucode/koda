...
const layoutPersistence = createAsyncStoragePersistence(AsyncStorageKeys.customLayouts, LayoutStoreSchema)
...
export const layoutStore = createStore({
  ...
  on: {
    ...
    updateLayout: (context, event: { id: string; layout: Partial<CustomLayout> }) => ({
      ...context,
      layouts: context.layouts.map(l => (l.id === event.id ? { ...l, ...event.layout } : l)),
    }),
...
export const layoutStoreHelpers = {
  addLayout: (layout: CustomLayout) => layoutStore.send({ type: 'addLayout', layout }),

  updateLayout: (id: string, layout: Partial<CustomLayout>) => layoutStore.send({ type: 'updateLayout', id, layout }),

  deleteLayout: (id: string) => layoutStore.send({ type: 'deleteLayout', id }),

  setDefaultLayout: (id: string) => layoutStore.send({ type: 'setDefaultLayout', id }),

  reorderLayouts: (layouts: CustomLayout[]) => layoutStore.send({ type: 'reorderLayouts', layouts }),
...
// Selector functions
export const selectLayouts = (state: ReturnType<typeof layoutStore.get>) => state.context.layouts

export const selectDefaultLayout = (state: ReturnType<typeof layoutStore.get>) =>
  state.context.layouts.find(l => l.isDefault)

export const selectLayoutById = (id: string) => (state: ReturnType<typeof layoutStore.get>) =>

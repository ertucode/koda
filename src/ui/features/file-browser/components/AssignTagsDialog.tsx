...
import { tagsStore, TAG_COLORS, TAG_COLOR_CLASSES, selectTagConfig, selectFileTags, type TagColor } from '../tags'
...
export function AssignTagsDialog({ fullPath }: { fullPath: string }) {
  ...
  return (
    <Dialog title={`Assign Tags to "${fileName}"`} onClose={dialogActions.close}>
      <div className="flex flex-col gap-2 min-w-[300px]">
        <p className="text-sm text-gray-500 mb-2">Select tags to assign to this item:</p>
        <div className="flex flex-col gap-1">
          {TAG_COLORS.map(color => {
            ...
            return (
              ...
              >
                <span className={clsx('size-4 rounded-full', colorClasses.dot)} />
                <span className={clsx('flex-1 text-left', isSelected && colorClasses.text)}>{tagName}</span>
                {isSelected && <CheckIcon className={clsx('size-4', colorClasses.text)} />}
              </button>
            )
          })}
        </div>

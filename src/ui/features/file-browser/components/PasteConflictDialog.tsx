...
import type { PasteConflictData, ConflictResolution } from '@common/Contracts'
...
export const PasteConflictDialog = (item: PasteConflictDialogMetadata) => {
  const [globalStrategy, setGlobalStrategy] = useState<'override' | 'trash' | 'autoName' | 'skip'>('autoName')
  ...
  useEffect(() => {
    if (item?.conflictData) {
      ...
      const initialActions = new Map<string, { action: PerFileAction; customName?: string }>()
  ...
  function checkDuplicateDestinations(): Map<string, number> {
    ...
    item.conflictData.conflicts.forEach(conflict => {
      ...
      if (action?.action === 'customName' && action.customName) {
        const dir = conflict.destinationPath.substring(0, conflict.destinationPath.lastIndexOf('/'))
        finalPath = `${dir}/${action.customName}`
      } else if (action?.action === 'skip') {
        return // Skip this check
      } else if (action?.action === 'useGlobal' || action?.action === 'override') {
  ...
  const duplicates = useMemo(() => checkDuplicateDestinations(), [perFileActions, globalStrategy, item?.conflictData])
  ...
  const hasErrors = validationErrors.size > 0 || duplicates.size > 0
  ...
  function updatePerFileAction(destPath: string, action: PerFileAction, customName?: string) {
      ...
      <Dialog
        ...
        footer={
          <>
            ...
            <Button onClick={handleSinglePaste} disabled={!action.customName?.trim() || !!error}>
              Paste
            </Button>
          </>
        }
      >
        <div className="flex flex-col gap-4">
          <p className="text-sm text-base-content/70">
            <span className="font-medium text-base-content">{fileName}</span> already exists. Enter a new name:
          </p>
          <div className="form-control">
            <input
              ...
              onChange={e => {
                updatePerFileAction(conflict.destinationPath, 'customName', e.target.value)
              }}
      ...
      <div className="flex flex-col gap-3">
        ...
        {exceedsLimit && (
          <div className="alert alert-warning">
            ...
            <span>
              More than 20 conflicts detected (showing 20 of {totalConflicts}). Select a global strategy to handle all
              conflicts.
            </span>
          </div>
        )}
        ...
        <div className="form-control border rounded-lg p-3 bg-base-200">
          <label className="label py-1">
            <span className="label-text font-semibold">Apply to all conflicts:</span>
          </label>
              ...
              {conflicts.map((conflict, idx) => {
                ...
                return (
                  <tr key={conflict.destinationPath}>
                    ...
                    <td className="font-mono text-[11px]">{conflict.sourcePath}</td>
                    <td className="text-xs text-right">{conflict.sourceSizeStr}</td>
                    <td className="text-xs text-right">{conflict.destSizeStr}</td>
                    <td>
                      <select
                        ...
                        onChange={e => {
                          ...
                          updatePerFileAction(
                            conflict.destinationPath,
                            newAction,
                            newAction === 'customName' ? conflict.suggestedName : undefined
                          )
                        }}
                      >
                    ...
                    <td>
                      <div className="flex flex-col gap-1">
                        <input
                          type="text"
                          className={`input input-bordered input-xs w-full ${error ? 'input-error' : ''}`}
                          value={action.customName || ''}
                          disabled={action.action !== 'customName'}
                          onChange={e => {
                            updatePerFileAction(conflict.destinationPath, 'customName', e.target.value)
                          }}
                          placeholder={action.action === 'customName' ? 'Enter new name' : ''}
                        />

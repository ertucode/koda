...
import { EyeIcon, EyeOffIcon, RefreshCcwIcon, ArrowUpAZIcon, ArrowDownAZIcon } from 'lucide-react'
...
export function ColumnHeaderContextMenu({ columns, directoryPath }: ColumnHeaderContextMenuProps) {
  const globalPrefs = useSelector(columnPreferencesStore, selectGlobalPreferences)
  const pathPrefs = useSelector(columnPreferencesStore, selectPathPreferences(directoryPath))
  const globalSort = useSelector(columnPreferencesStore, selectGlobalSort)
  const pathSort = useSelector(columnPreferencesStore, selectPathSort(directoryPath))
  ...
  const getOrderedColumns = (prefs: typeof pathPrefs | typeof globalPrefs) => {
    ...
    const ordered: ColumnRowProps['column'][] = [...prefs]
      ...
      .map(p => {
        const col = columns.find(c => c.id.toString() === p.id)
        return {
          id: p.id,
          visible: p.visible,
  ...
  const togglePathSort = (sortKey: string | number | undefined) => {
    ...
    const newSort: SortState = {
      by: p.data,
      order: pathSort?.by === p.data ? (pathSort.order === 'asc' ? 'desc' : 'asc') : 'asc',
    }
  ...
  const toggleGlobalSort = (sortKey: string | number | undefined) => {
    ...
    const newSort: SortState = {
      by: p.data,
      order: globalSort?.by === p.data ? (globalSort.order === 'asc' ? 'desc' : 'asc') : 'asc',
    }
  ...
  const isPathRefreshEnabled = (pathPrefs && pathPrefs.length > 0) || !!pathSort
  const isGlobalRefreshEnabled = (globalPrefs && globalPrefs.length > 0) || !!globalSort

  return (
    <div className="bg-base-200 rounded-xl shadow-4xl w-80 overflow-hidden">
      <h2 className="flex items-center justify-center p-2 bg-base-300">Column Preferences</h2>
      <div className="flex text-xs">
        {/* Path Preferences */}
        <div className="flex-1 flex flex-col min-w-0">
          <div className="text-xs font-semibold border-b border-base-300 p-3 pl-3.5">For Directory Path</div>
        ...
        {/* Global Preferences */}
        <div className="flex-1 flex flex-col min-w-0">
          <div className="text-xs font-semibold border-b border-base-300 p-3 pl-3.5">Global</div>
          <div className="space-y-0.5 max-h-96 overflow-y-auto flex-1">
...
function ColumnRow({ column, index, onToggle, onMove, onSort, currentSort }: ColumnRowProps) {
  ...
  return (
    ...
    >
      ...
      >
        {column.visible ? <EyeIcon className="size-3" /> : <EyeOffIcon className="size-3 opacity-50" />}
      </button>
      <span className={`flex-1 truncate text-xs ${!column.visible ? 'opacity-50 line-through' : ''}`}>
        {column.header}
      </span>
      {column.sortKey &&
        currentSort?.by === column.sortKey &&
        (currentSort.order === 'asc' ? <ArrowDownAZIcon className="size-4" /> : <ArrowUpAZIcon className="size-4" />)}
    </div>
  )
}

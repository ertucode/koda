...
function getTagStateForFiles(fullPaths: string[], color: TagColor, fileTags: FileTags): TagState {
...
export function MultiFileTagsDialog({ fullPaths }: { fullPaths: string[] }) {
  const fileTags = useSelector(tagsStore, selectFileTags)
  const tagConfig = useSelector(tagsStore, selectTagConfig)

  if (!fullPaths || fullPaths.length === 0) return null
  ...
  const handleHeaderClick = (color: TagColor) => {
    ...
    if (state === 'all') {
      // Remove from all
      fullPaths.forEach(path => tagsStore.send({ type: 'removeTagFromFile', fullPath: path, color }))
    } else {
      // Add to all
  ...
  return (
    <Dialog title={`Assign Tags to ${fullPaths.length} Items`} onClose={dialogActions.close}>
      <div className="flex flex-col gap-3 min-w-[500px] max-w-[800px] max-h-[60vh]">
        <p className="text-sm text-gray-500">
          Click cells to toggle tags. Click column headers to toggle for all files.
        </p>
        ...
        <div className="overflow-auto border border-base-300 rounded-lg">
          <table className="table table-sm w-full">
            <thead className="sticky top-0 bg-base-200 z-10">
              <tr>
                <th className="text-left font-medium py-2 px-3 min-w-[150px]">File</th>
                  ...
                  return (
                    ...
                    >
                      <div className="flex flex-col items-center gap-1">
                        <span className={clsx('size-3 rounded-full', colorClasses.dot)} />
                        <span className="text-xs truncate max-w-[70px]">{tagName}</span>
                        {state !== 'none' && (
                          <div
                            className={clsx(
                          ...
                          >
                            {state === 'all' ? (
                              <CheckIcon className={clsx('size-3', colorClasses.text)} />
                            ) : (
            ...
            <tbody>
              {fullPaths.map(fullPath => {
                ...
                return (
                  <tr key={fullPath} className="hover:bg-base-200/50 border-t border-base-300">
                    <td className="py-2 px-3 font-mono text-sm truncate max-w-[200px]" title={fullPath}>
                      {fileName}
                    </td>
                    {TAG_COLORS.map(color => {

...
import z from 'zod'
import { SaveIcon, FolderOpenIcon, PlayIcon, RotateCcwIcon, HistoryIcon } from 'lucide-react'
import { getWindowElectron } from '@/getWindowElectron'
import { useAsyncStorage } from '@/lib/hooks/useAsyncStorage'

type BatchRenameTemplate = {
  name: string
  options: Omit<BatchRenameOptions, 'counterStart' | 'counterStep' | 'counterPadding'> & {
    counterStart: number
    counterStep: number
    ...
    >
      <div className="flex flex-col gap-3">
        <div className="text-sm opacity-70">
          Showing the last {undoHistory.length} batch rename operations (most recent first). You can undo the latest
          operation from the main dialog.
        </div>
            ...
            return (
              <div key={actualIdx} className="border rounded-lg p-3 bg-base-200">
                <div className="flex justify-between items-center mb-2">
                  <span className="text-sm font-semibold">Operation #{actualIdx + 1}</span>
                  <div className="flex items-center gap-2">
                    <span className="text-xs opacity-60">
                      {operation.length} file{operation.length !== 1 ? 's' : ''}
                    </span>
                    {actualIdx === undoHistory.length - 1 && (
                      <span className="badge badge-primary badge-sm">Latest - Can Undo</span>
                    )}
                  </div>
                </div>
                  ...
                  <table className="table table-zebra table-xs">
                    ...
                    <tbody>
                      {operation.map((rename, idx) => {
                        const oldName = rename.oldPath.substring(rename.oldPath.lastIndexOf('/') + 1)
                        const newName = rename.newPath.substring(rename.newPath.lastIndexOf('/') + 1)
                        return (
                          <tr key={idx}>
...
export const BatchRenameDialog = ({ ref }: { ref: Ref<DialogForItem<GetFilesAndFoldersInDirectoryItem[]>> }) => {
  ...
  const insertPlaceholder = (placeholder: string) => {
    ...
    const newMask = currentMask.substring(0, start) + placeholder + currentMask.substring(end)
  ...
  const previews = useMemo<RenamePreview[]>(() => {
    ...
    items.forEach(item => {
      item.fullPath = directoryHelpers.getFullPathForItem(item, directoryStore.getSnapshot().context.activeDirectoryId)
    })
  ...
  const unchangedCount = previews.filter(p => p.oldName === p.newName && !p.error).length

  async function handleRename() {
    if (!items || hasErrors) return

    const activeDirectoryId = directoryStore.getSnapshot().context.activeDirectoryId
    ...
    try {
      ...
      onResult(GenericError.Message(error instanceof Error ? error.message : 'Failed to rename files'), {})
    }
  }
  ...
  function saveTemplate() {
    ...
    setTemplates(prev => [...prev.filter(t => t.name !== templateName), template])
    setTemplateName('')
  }
  ...
  async function handleUndo() {
    ...
    try {
      ...
      onResult(GenericError.Message(error instanceof Error ? error.message : 'Failed to undo'), {})
    }
  }
      ...
      <Dialog
        ...
        footer={
          <>
            {undoHistory.length > 0 && (
              <>
                <Button onClick={() => setShowHistory(true)} icon={HistoryIcon}>
                  View History
                </Button>
            ...
            <Button onClick={handleRename} icon={PlayIcon} disabled={hasErrors || !items || items.length === 0}>
        ...
        <div className="flex flex-col gap-3">
          ...
          <div>
            <label className="label py-1">
              <span className="label-text text-xs font-semibold">Rename Mask</span>
            ...
            <div className="flex gap-2">
              ...
              <div className="flex-1 form-control">
                <input
                  ...
                  type="text"
                  value={options.mask}
                  onChange={e => setOptions(prev => ({ ...prev, mask: e.target.value }))}
                  className="input input-bordered input-sm"
                  placeholder="[N] - [C].[E]"
                />
                <div className="flex flex-wrap gap-1 mt-1">
                  <button onClick={() => insertPlaceholder('[N]')} className="btn btn-xs btn-ghost" type="button">
                    [N] Name
                  </button>
                  <button onClick={() => insertPlaceholder('[N1-5]')} className="btn btn-xs btn-ghost" type="button">
                    [N1-5] Chars
                  </button>
                  <button onClick={() => insertPlaceholder('[E]')} className="btn btn-xs btn-ghost" type="button">
                    [E] Ext
                  </button>
                  <button onClick={() => insertPlaceholder('[C]')} className="btn btn-xs btn-ghost" type="button">
                    [C] Counter
                  </button>
                  <button onClick={() => insertPlaceholder('[d]')} className="btn btn-xs btn-ghost" type="button">
                    [d] Date
                  </button>
                  <button onClick={() => insertPlaceholder('[t]')} className="btn btn-xs btn-ghost" type="button">
                    [t] Time
                  </button>
                  <button onClick={() => insertPlaceholder('[P]')} className="btn btn-xs btn-ghost" type="button">
                    [P] Parent
                  </button>
                </div>
              </div>
          ...
          <div className="form-control">
            ...
            <div className="flex justify-between items-start mt-1">
              ...
              <div className="text-xs opacity-60">Masks & capture groups ($1, $2) supported</div>
            </div>
          </div>
          ...
          <div className="form-control">
            <label className="label py-1">
              <span className="label-text text-xs font-semibold">Case Conversion</span>
            </label>
            <select
          ...
          <div className="flex flex-col gap-2 border-t pt-3">
            <div className="flex gap-2">
              ...
              />
              <Button onClick={saveTemplate} icon={SaveIcon} disabled={!templateName.trim()}>
                Save
              </Button>
              <Button onClick={() => setShowTemplates(!showTemplates)} icon={FolderOpenIcon}>
                ...
                {templates.map(template => (
                  ...
                  >
                    ...
                    <div className="flex gap-1">
                      <button onClick={() => loadTemplate(template)} className="btn btn-xs btn-primary">
                        Load
                      </button>
                      <button onClick={() => deleteTemplate(template.name)} className="btn btn-xs btn-error">
                        Delete
                      </button>
                    </div>
                  </div>
          ...
          <div className="flex flex-col gap-1 border-t pt-3">
            <div className="flex justify-between items-center">
              <label className="text-xs font-semibold">Preview ({previews.length} items)</label>
              <div className="text-xs">
                {hasErrors && <span className="text-error mr-2 font-semibold">Has errors!</span>}
                {unchangedCount > 0 && <span className="opacity-60">{unchangedCount} unchanged</span>}
              </div>
            </div>
                ...
                <tbody>
                  {previews.map((preview, idx) => (
                    <tr key={idx} className={preview.error ? 'bg-error/10' : ''}>
                      ...
                      <td>
                        {preview.error ? (
                          <span className="badge badge-error badge-xs">{preview.error}</span>
                        ) : preview.oldName === preview.newName ? (
                          <span className="badge badge-ghost badge-xs">Unchanged</span>
                        ) : (

...
import { SearchIcon, XIcon, FileIcon, ChevronDownIcon, ChevronUpIcon, Replace, ReplaceAll } from 'lucide-react'
...
export function StringFinderTab({ onClose }: StringFinderTabProps) {
  ...
  }, [directory, query, cwd, includePatterns, excludePatterns, useRegex, caseSensitive, searchHidden])
  ...
  const handleReplace = async (replaceAll: boolean) => {
    ...
    try {
      if (replaceAll) {
        ...
        const uniqueFiles = Array.from(new Set(searchResults.map(r => r.filePath))).map(filePath => ({ filePath }))
        ...
        if (result.success) {
          const totalReplacements = result.data.reduce((sum, r) => sum + r.replacementCount, 0)
          setReplaceStatus(`Replaced ${totalReplacements} occurrence(s) in ${result.data.length} file(s)`)
  ...
  const highlightReplacement = (text: string, query: string, replacement: string) => {
    ...
    return (
      <>
        {text.slice(0, index)}
        <span className="bg-red-200 text-red-800 line-through">{text.slice(index, index + query.length)}</span>
        <span className="bg-green-200 text-green-800">{replacement}</span>
        {text.slice(index + query.length)}
      </>
    )
  }
    ...
    <div className="flex gap-3 h-full overflow-visible">
      ...
      <div className="flex flex-col gap-3 w-[350px] min-w-[300px] h-full flex-shrink-0">
        ...
        <div className="flex flex-col gap-2 flex-shrink-0">
          ...
          {replaceStatus && <div className="text-xs text-success bg-success/10 px-2 py-1 rounded">{replaceStatus}</div>}
          ...
          {showAdvanced && (
            <div className="flex flex-col gap-3 p-3 bg-base-200 rounded-lg text-xs border border-base-300">
              {/* Directory / CWD */}
              <div>
                <label className="block font-medium text-base-content mb-1">Directory</label>
                <input
                  type="text"
                  value={cwd}
                ...
                />
                <p className="text-base-content/60 mt-1">Relative to current dir, or absolute if starts with / or ~</p>
              </div>

              {/* Include patterns */}
              <div>
                <label className="block font-medium text-base-content mb-1">Include files</label>
                <input
                  type="text"
                  ...
                  className="input input-bordered input-sm w-full bg-base-100"
                />
                <p className="text-base-content/60 mt-1">Glob patterns, comma-separated</p>
              </div>

              {/* Exclude patterns */}
              <div>
                <label className="block font-medium text-base-content mb-1">Exclude files</label>
                <input
                  type="text"
        ...
        <div className="flex-1 min-h-0 border border-gray-200 rounded overflow-hidden flex flex-col">
          {isLoading && <div className="text-center text-gray-500 py-4">Searching...</div>}
              ...
              {searchResults.length === 0 ? (
                ...
                <div>
                  {searchResults.map((result, index) => {
                    const { fileName, folder } = getFileNameAndFolder(result.filePath)
                    return (
                      <div
                      ...
                      >
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-xs text-gray-400">:{result.matchLineNumber}</span>
                          {folder && <span className="text-xs text-gray-500 truncate ml-2">{folder}</span>}
                        </div>
        ...
        <div className="text-xs text-gray-500 flex flex-wrap gap-x-4 gap-y-1 justify-center flex-shrink-0">
          <div>
            <kbd className="kbd">↑↓</kbd> or <kbd className="kbd">Ctrl+J/K</kbd> navigate
          </div>
          <div>
        ...
        {selectedResult ? (
          <>
            ...
            <div className="flex items-center gap-2 pb-2 border-b border-gray-200 flex-shrink-0">
              ...
              <span className="text-xs text-gray-400 flex-shrink-0">Line {selectedResult.matchLineNumber}</span>
          ...
          <div className="flex-1 flex items-center justify-center text-gray-400 text-sm">
            {query ? 'Select a result to see context' : 'Search results will appear here'}
          </div>
        )}
      </div>
    </div>

...
export const TAG_COLORS = ['red', 'orange', 'yellow', 'green', 'blue', 'purple', 'pink'] as const
...
const tagPersistence = createAsyncStoragePersistence(AsyncStorageKeys.tags, tagsSchema)
...
export const tagsStore = createStore({
  ...
  on: {
    ...
    addTagToFiles: (context, event: { fullPaths: string[]; color: TagColor }) => {
    ...
    removeTagFromFile: (context, event: { fullPath: string; color: TagColor }) => {
      const currentTags = context.fileTags[event.fullPath] || []
    ...
    toggleTagOnFile: (context, event: { fullPath: string; color: TagColor }) => {
      const currentTags = context.fileTags[event.fullPath] || []
      if (currentTags.includes(event.color)) {
        const newTags = currentTags.filter((c: TagColor) => c !== event.color)
        const { [event.fullPath]: _, ...rest } = context.fileTags
    ...
    toggleTagOnFiles: (context, event: { fullPaths: string[]; color: TagColor }) => {
      const newFileTags = { ...context.fileTags }
...
export const selectTagConfig = (state: ReturnType<typeof tagsStore.get>) => state.context.tagConfig
export const selectFileTags = (state: ReturnType<typeof tagsStore.get>) => state.context.fileTags
export const selectLastUsedTag = (state: ReturnType<typeof tagsStore.get>) => state.context.lastUsedTag

// Computed selectors
export const selectTagName = (color: TagColor) => (state: ReturnType<typeof tagsStore.get>) =>
  state.context.tagConfig[color] || color

export const selectFileTagsForPath = (fullPath: string) => (state: ReturnType<typeof tagsStore.get>) =>
  state.context.fileTags[fullPath] || []

export const selectFilesWithTag = (color: TagColor) => (state: ReturnType<typeof tagsStore.get>) =>
  Object.entries(state.context.fileTags)
    .filter(([, tags]) => tags.includes(color))
    .map(([path]) => path)

export const selectFileCountWithTag = (color: TagColor) => (state: ReturnType<typeof tagsStore.get>) =>
  Object.entries(state.context.fileTags).filter(([, tags]) => tags.includes(color)).length

export const selectHasTag = (fullPath: string, color: TagColor) => (state: ReturnType<typeof tagsStore.get>) =>
  (state.context.fileTags[fullPath] || []).includes(color)

export const selectEveryFileHasSameTags = (fullPaths: string[]) => (state: ReturnType<typeof tagsStore.get>) => {

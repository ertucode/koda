...
export const recentsStore = createStore({
  ...
  on: {
    addRecent: (context, event: { item: Omit<RecentItem, 'timestamp'> }) => {
      const isLastItemEquivalent = context.recents.length > 0 && context.recents[0].fullPath === event.item.fullPath
      if (isLastItemEquivalent) return context

      // Remove if already exists
      const filtered = context.recents.filter(recent => recent.fullPath !== event.item.fullPath)
    ...
    removeRecent: (context, event: { fullPath: string }) => ({
      ...context,
      recents: context.recents.filter(recent => recent.fullPath !== event.fullPath),
    }),
...
export const selectRecents = (state: ReturnType<typeof recentsStore.get>) => state.context.recents
